...
public:
    ...
    void addClient()
    {
        while (true)
        {
            ...
            client_count.store(client_count.load() + 1);
            std::cout << "A client has come. Current client number: " << client_count.load() << std::endl;
        }
    }
    ...
    void serveClient()
    {
        while (true)
        {
            ...
            if (clients_done && client_count.load() == 0)
            {
                break;
            }
            ...
            if (client_count.load() > 0)
            {
                ...
                client_count.store(client_count.load() - 1);
                std::cout << "A client is served. Remain: " << client_count.load() << std::endl;
            }
        }
